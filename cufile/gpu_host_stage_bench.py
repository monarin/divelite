...
def _detect_cufile_lib() -> Optional[str]:
    found = ctypes.util.find_library("cufile")
    if found:
        return found
    candidates = (
        "/usr/lib64/libcufile.so",
...
def _nvidia_fs_loaded() -> Optional[bool]:
    try:
        with open("/proc/modules", "r", encoding="utf-8") as fobj:
            for line in fobj:
...
def _build_common_result(
    args: argparse.Namespace, file_size: int, max_bytes_per_pass: int
) -> Dict[str, Any]:
    return {
        "tool": "gpu_host_stage_bench",
        "mode": args.mode,
        "path": args.path,
        "file_size_bytes": int(file_size),
        "file_size_gib": float(file_size / GIB),
        "chunk_mb": float(args.chunk_mb),
        "chunk_bytes": int(args.chunk_mb * 1024 * 1024),
        "max_gb_per_pass": float(args.max_gb),
        "bytes_per_pass": int(max_bytes_per_pass),
        "passes": int(args.passes),
        "warmup_chunks": int(max(0, args.warmup_chunks)),
        "gpu": int(args.gpu),
    }


def _write_json_result(path: Optional[str], payload: Dict[str, Any]) -> None:
    if not path:
        return
...
def _run_cufile_mode(
    ...
    kd_num_threads = "unknown"
    kd_task_size = "unknown"
    try:
        import kvikio.defaults as kd

        kd_num_threads = kd.get("num_threads")
        kd_task_size = kd.get("task_size")
    except Exception:
        pass

    cp.cuda.runtime.setDevice(args.gpu)
    cuda_runtime_ver = _try_int(cp.cuda.runtime.runtimeGetVersion)
    cuda_driver_ver = _try_int(cp.cuda.runtime.driverGetVersion)
    ...
    if cufile_driver is not None:
        for key in ("is_gds_available", "major_version", "minor_version", "driver_version"):
            try:
                cufile_info[key] = str(cufile_driver.get(key))
            except Exception:
                continue
        if "is_gds_available" in cufile_info:
            gds_available = cufile_info["is_gds_available"]
    ...
    print(f"  cupy_version    : {getattr(cp, '__version__', 'unknown')}")
    print(f"  kvikio_version  : {getattr(kvikio, '__version__', 'unknown')}")
    print(f"  libcufile       : {_detect_cufile_lib() or 'not found'}")
    print(f"  nvidia_fs_loaded: {_nvidia_fs_loaded()}")
    print(f"  cuda_runtime    : {cuda_runtime_ver}")
    print(f"  cuda_driver     : {cuda_driver_ver}")
    print(f"  KVIKIO_NTHREADS : {os.environ.get('KVIKIO_NTHREADS', '(default)')}")
    print(f"  KVIKIO_TASK_SIZE: {os.environ.get('KVIKIO_TASK_SIZE', '(default)')}")
    print(f"  kd.num_threads  : {kd_num_threads}")
    print(f"  kd.task_size    : {kd_task_size}")
    if cufile_info:
        print(f"  cufile_driver   : {cufile_info}")
    print()
    ...
    result = _build_common_result(args=args, file_size=file_size, max_bytes_per_pass=max_bytes_per_pass)
    result.update(
        {
            "backend": "kvikio+cupy",
            "cufile_compat": compat_state,
            "gds_available": gds_available,
            "cufile_driver": cufile_info,
            "cupy_version": getattr(cp, "__version__", "unknown"),
            "kvikio_version": getattr(kvikio, "__version__", "unknown"),
            "libcufile": _detect_cufile_lib(),
            "nvidia_fs_loaded": _nvidia_fs_loaded(),
            "cuda_runtime_version": cuda_runtime_ver,
            "cuda_driver_version": cuda_driver_ver,
            "measured_data_bytes": int(stats.measured_bytes),
            "measured_chunks": int(stats.measured_chunks),
            "cufile_io_time_s": float(stats.read_seconds),
            "wall_time_s": float(wall_seconds),
            "cufile_throughput_gib_s": float(io_gibs),
            "end_to_end_gib_s": float(e2e_gibs),
        }
    )
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional path for JSON result output (useful for scripted comparisons). "
            "If omitted, no JSON file is written."
        ),
    )
    ...
    result.update(
        {
            ...
            "measured_data_bytes": int(stats.measured_bytes),
            "measured_chunks": int(stats.measured_chunks),
            "read_time_s": float(stats.read_seconds),
            "h2d_copy_time_s": float(stats.copy_seconds),
            "wall_time_s": float(wall_seconds),
            "read_throughput_gib_s": float(read_gibs),
            "h2d_throughput_gib_s": float(copy_gibs),
            "end_to_end_gib_s": float(e2e_gibs),
        }
    )
    _write_json_result(args.json_out, result)
