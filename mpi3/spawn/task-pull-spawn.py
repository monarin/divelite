...
if len(sys.argv) == 1:
    ...
    # Append stop sentinetl for each worker
    ...
    for position, msg in enumerate(msg_list):
        ...
        comm.send(obj=msg, dest=dest)
        print(f'send {position=} {msg=} to {dest=}', flush=True)
    ...
    #comm.gather(reports, root=MPI.ROOT)
    ...
    print('\n\n  Worker   Tasks    Time')
    print('-' * 26)
    for worker, report in enumerate(log):
    ...
    log = []
    for task in iter(lambda: comm.sendrecv(rank, dest=0), StopIteration):
        log.append(task)
