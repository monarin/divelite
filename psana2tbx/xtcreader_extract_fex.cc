...
int main(int argc, char* argv[])
{
    ...
    // Looping over the same file to write out multiple L1Accept
    int nloop=1000;
    for (int i=0; i<nloop; i++) {
        ...
        while ((dg = iter.next())) {
            ...
            // Only write out if this is an L1 or Transition in the first loop
            if ( (dg->service()==TransitionId::L1Accept) || 
                    (dg->service()==TransitionId::SlowUpdate) ||
                    (i == 0 && (dg->service()==TransitionId::Configure ||
                                dg->service()==TransitionId::BeginRun ||
                                dg->service()==TransitionId::BeginStep ||
                                dg->service()==TransitionId::Enable)) ||
                    (i == nloop-1 && (dg->service()==TransitionId::EndRun ||
                                dg->service()==TransitionId::EndStep ||
                                dg->service()==TransitionId::Disable))
               )
            {
                ...
                if (fwrite(dg, sizeof(*dg) + dg->xtc.sizeofPayload(), 1, outfile) != 1) {
                    ...
                    printf("Write evt:%d service:%d ts:%u\n", nevent, dg->service(), counting_timestamp);
                }
